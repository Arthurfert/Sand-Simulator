...
public:
    ...
    int getV() const; // Accesseur pour v
    void setPosition(int x, int y); // Modificateur pour x et y

    void update(class Table& table, int x, int y, bool vide, bool inertie) override; // Ajout de override

private:
    int x; // Coordonnée x du sable
    int y; // Coordonnée y du sable
    int v = 3; // vitesse (pour l'inertie)
};
